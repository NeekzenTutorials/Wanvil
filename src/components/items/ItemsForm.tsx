...
export function ItemsForm({ itemId, collectionId, onClose }:{
  ...
  useEffect(() => {
    Promise.all([
      apiGet<any>(`items/${itemId}`),
      apiGet<Tag[]>(`collections/${collectionId}/tags?scope=item`)
    ]).then(([it, tags]) => {
      // Normalise le shape
      ...
      setData({ ...it, content })
      setAllTags(tags)
      setSelectedTagIds((it.tags || []).map((t:any) => t.id))
    })
  }, [itemId, collectionId])
  ...
  const removeImage = (i:number) =>
    setData((prev:any) => ({ ...prev, images: (prev.images || []).filter((_:any, idx:number) => idx !== i) }))

  // --- Champs personnalisés par Objet --------------------------------------
  const getCustomFields = (): CustomField[] => data?.content?.customFields || []
  const setCustomFields = (fields: CustomField[]) =>
    setData((prev:any) => ({ ...prev, content: { ...(prev.content||{}), customFields: fields } }))
  ...
  const removeCustomField = (id:string) =>
    setCustomFields(getCustomFields().filter(f => f.id !== id))
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                onClick={()=>{
                  const el = document.getElementById('item-img-url') as HTMLInputElement
                  addImage(el.value)
                  if (el) el.value = ''
                }}
              >
