// src/components/places/PlacesTagsManagerPage.tsx
import { useEffect, useState } from 'react'
import { apiGet, apiPost, apiPut, apiDelete } from '../../utils/fetcher'

type Collection = { id: string; name: string }
type Tag = { id: string; name: string; color?: string; note?: string; collectionId: string; scope: 'place'|'character' }

export function PlacesTagsManagerPage({ projectId }: { projectId: string }) {
  ...
  const createTag = async () => {
    ...
    const t = await apiPost<Tag>(`collections/${collectionId}/tags`, {
      name: draft.name.trim(), color: draft.color || null, note: draft.note || null, scope: 'place'
    })
    setTags(prev => [t, ...prev])
  ...
  const saveTag = async (tag: Tag) => {
    ...
    try {
      const updated = await apiPut<Tag>(`tags/${tag.id}`, {
        name: tag.name, color: tag.color, note: tag.note ?? null, scope: 'place'
      })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border bg-white p-4 shadow-sm">
        <h3 className="font-semibold mb-3">Créer un tag (Lieux)</h3>
      ...
      <div className="rounded-xl border bg-white p-4 shadow-sm">
        <h3 className="font-semibold mb-3">Tags (Lieux) de la collection</h3>
