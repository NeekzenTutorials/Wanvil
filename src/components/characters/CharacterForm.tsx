...
function Field({ field, value, onChange }:{ field:TemplateField; value:any; onChange:(v:any)=>void }) {
  ...
  if (field.type === 'richtext') return <Labeled label={field.label}>
    <Editor licenseKey='gpl' tinymceScriptSrc="/tinymce/tinymce.min.js" value={value||''} onEditorChange={onChange} init={{ base_url: '/tinymce', suffix: '.min',menubar:false, height:300, browser_spellcheck: true, contextmenu: false, plugins:'link lists table', toolbar:'bold italic | bullist numlist | link table' }} />
  </Labeled>
