...
export function CharacterTagsManagerPage({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    ...
    apiGet<Tag[]>(`collections/${collectionId}/tags`).then(setTags)
  }, [collectionId])

  const createTag = async () => {
    if (!collectionId || !draft.name.trim()) return
    const t = await apiPost<Tag>(`collections/${collectionId}/tags`, {
      name: draft.name.trim(), color: draft.color || null, note: draft.note || null
    })
    setTags(prev => [t, ...prev])
  ...
  const saveTag = async (tag: Tag) => {
    setSavingId(tag.id)
    try {
      const updated = await apiPut<Tag>(`tags/${tag.id}`, { name: tag.name, color: tag.color, note: tag.note ?? null })
